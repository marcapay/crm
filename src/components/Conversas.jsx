...
export default function Conversas() {
  ...
  const [starredClients, setStarredClients] = useState(() => {
    const saved = localStorage.getItem('analise_starred_clients');
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    localStorage.setItem('analise_starred_clients', JSON.stringify(starredClients));
  }, [starredClients]);
  ...
  const activeClient = clients.find(c => c.id === activeChatClientId) || clients[0];
  ...
  // Filter and sort clients by most recent message timestamp
  ...
  const handleFileChange = (e) => {
    ...
    // Check size to prevent localStorage overflow
              ...
              >
                <div style={styles.contactLeft}>
                  ...
                  <div style={styles.contactInfo}>
                    ...
                    <span style={styles.contactLastMsg}>
                      {lastMsg ? lastMsg.text : (client.lastMessageText || 'Sem mensagens ainda.')}
                    </span>
                  </div>
                </div>
                ...
                <div style={styles.contactRight}>
                  <span style={styles.contactTime}>
                    {lastMsg ? lastMsg.time : (client.lastMessageTime || '')}
                  </span>
          ...
          <>
            ...
            <div style={styles.chatHeader}>
              <div style={styles.chatHeaderLeft}>
                ...
                <div>
                  <h3 style={styles.chatHeaderName}>{activeClient.name}</h3>
                  <div style={styles.chatHeaderSub}>
                    <span>{activeClient.email || activeClient.phone}</span>
                  </div>
                </div>
              </div>
                ...
                <div ref={transferMenuRef} style={{ position: 'relative' }}>
                  ...
                  {showTransferMenu && (
                    <div style={styles.transferDropdown}>
                      <div style={styles.transferHeader}>Transferir para:</div>
                      {(() => {
